...
function arenaInfo(){const mini=document.querySelector('#travel2Mini'),a=document.querySelector('#travel2Arena');if(!mini||!a)return null;const title=(mini.querySelector('h4')?.textContent||'').toLowerCase();return{a,title}}
...
function arrange(){const info=arenaInfo();if(!info)return;const {a,title}=info;const items=[...a.querySelectorAll('.t2-item:not(.hit)')];if(!items.length)return;const old=state.get(a),kind=isCoffee(title)?'coffee':isBird(title)?'bird':isTrash(title)?'trash':'other';if(!old||old.kind!==kind){state.set(a,{kind});items.forEach(el=>{el.style.transition=kind==='bird'?'left .30s linear, top .30s linear, transform .15s ease':'transform .15s ease'});}
if(kind==='coffee'){items.slice(0,4).forEach((el,i)=>{el.style.zIndex=3;const r=a.getBoundingClientRect(),col=i%2,row=Math.floor(i/2),x=(col+.5)*r.width/2-el.offsetWidth/2,y=16+row*Math.max(52,r.height/3);el.style.left=Math.max(10,Math.min(r.width-el.offsetWidth-10,x))+'px';el.style.top=Math.min(r.height-el.offsetHeight-10,y)+'px'});items.slice(4).forEach(el=>el.remove())}
...
function boot(){const run=()=>arrange();run();new MutationObserver(run).observe(document.body,{childList:true,subtree:true});setInterval(run,280);window.addEventListener('resize',run)}
