...
function simplify(doc){
 ...
 if(nav){nav.querySelectorAll('#modeSwitch,.mode-switch,.child-return').forEach(el=>el.remove());const b=doc.createElement('a');b.className='child-return';b.href='index.html';b.target='_top';b.textContent='📘 Modo normal';b.addEventListener('click',()=>{try{localStorage.setItem('andina-explora-mode','normal')}catch(e){}});nav.appendChild(b)}
 doc.querySelectorAll('.child-note').forEach((el,i)=>{if(i>0)el.remove()});
 ...
 if(!doc.getElementById('childModeStyle')){const style=doc.createElement('style');style.id='childModeStyle';style.textContent=`
 ...
 html.modo-ninos-child .topic-grid,html.modo-ninos-child .info-cards,html.modo-ninos-child .culture-grid,html.modo-ninos-child .challenge-grid{gap:14px}
 ...
 if(!doc.__andinaChildObserver){
  ...
  doc.__andinaChildObserver=obs;
 }
}
if(frame){frame.addEventListener('load',()=>{try{simplify(frame.contentDocument);setTimeout(()=>loader&&loader.classList.add('hide'),450)}catch(e){console.error(e);if(loader)loader.classList.add('hide')}});}
})();