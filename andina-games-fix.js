...
const places=[
 ...
 {id:'cafe',name:'Eje Cafetero',tag:'montañas · café · paisaje cultural',image:images.paramo,
 ...
 {id:'paramo',name:'Páramo',tag:'alta montaña · agua · biodiversidad',image:images.cafe,
...
function init(){
 ...
 function setMap(){stage.innerHTML=`<div class="map-shell"><img class="map-image" ${imageSources(images.map)} alt="Mapa ilustrado de la Región Andina"><div id="hotspots"></div></div><div class="map-tip">📍 Selecciona una región para iniciar la siguiente misión.</div>`;const img=stage.querySelector('.map-image');img.addEventListener('error',()=>{if(img.dataset.fallback&&!img.src.includes('raw.githubusercontent.com'))img.src=img.dataset.fallback},{once:true});const hs=stage.querySelector('#hotspots');places.forEach(p=>{const b=document.createElement('button');b.className='map-hotspot'+(state.done.has(p.id)?' done':'');b.dataset.id=p.id;b.style.cssText=p.hotspot;b.textContent=p.name;b.disabled=state.done.has(p.id);b.addEventListener('click',()=>startRegion(p.id));hs.appendChild(b)})}
 function updateStatus(){livesEl.innerHTML=[0,1,2].map(i=>`<span class="${i<state.lives?'':'off'}">❤️</span>`).join('');livesEl.setAttribute('aria-label',`${state.lives} vidas restantes`);regionEl.textContent=`${state.completed} / 4 regiones`;const m=Math.floor(state.time/60),s=state.time%60;timerEl.textContent=`${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;timerEl.style.background=state.time<=30?'#f0d1c7':'#dced92'}
 ...
 function startRegion(id){if(state.locked||state.done.has(id))return;const p=places.find(x=>x.id===id);state.current=p;state.qIndex=0;state.questions=shuffle(p.questions);state.locked=false;renderQuestion();startTimer()}
 function renderQuestion(){const p=state.current,q=state.questions[state.qIndex];const options=q[1].map((text,i)=>({text,correct:i===q[2]}));const mixed=shuffle(options);stage.innerHTML=`<div class="mission-panel" style="background-image:url('${p.image.primary}'),url('${p.image.fallback}')"><div class="mission-content"><div class="mission-top"><h3>${p.name}</h3><span class="mission-tag">${p.tag}</span></div><div class="mission-question"><h4>${q[0]}</h4><div class="answer-grid"></div><div class="feedback" id="feedback"></div><div class="mission-footer"><span>Pregunta ${state.qIndex+1}/${state.questions.length}</span><div class="progress-bar"><i style="width:${((state.qIndex)/state.questions.length)*100}%"></i></div></div></div></div></div><button class="map-return" id="mapReturn">↩ Volver al mapa</button>`;const panel=stage.querySelector('.mission-panel');const img=new Image();img.onerror=()=>{if(!panel.dataset.fallback){panel.dataset.fallback='1';panel.style.backgroundImage=`url('${p.image.fallback}')`}};img.src=p.image.primary;const grid=stage.querySelector('.answer-grid');mixed.forEach((o,i)=>{const b=document.createElement('button');b.className='answer';b.style.animationDelay=`${i*55}ms`;b.textContent=o.text;b.addEventListener('click',()=>answer(b,o.correct));grid.appendChild(b)});stage.querySelector('#mapReturn').onclick=()=>{clearInterval(state.interval);setMap()}}
 function answer(button,correct){if(state.locked)return;state.locked=true;stage.querySelectorAll('.answer').forEach(b=>b.disabled=true);const feedback=stage.querySelector('#feedback');if(correct){button.classList.add('correct');feedback.className='feedback good';feedback.textContent='✓ ¡Correcto! Buen razonamiento territorial'}else{button.classList.add('wrong');state.lives--;updateStatus();feedback.className='feedback bad';feedback.textContent='✗ Incorrecto. Pierdes una vida';const q=state.questions[state.qIndex];stage.querySelectorAll('.answer').forEach(b=>{if(b.textContent===q[1][q[2]])b.classList.add('correct')});if(state.lives<=0){setTimeout(()=>loseGame('Te quedaste sin vidas. ¡La próxima expedición puede ser mejor!'),900);return}}setTimeout(()=>{state.locked=false;if(state.qIndex<state.questions.length-1){state.qIndex++;renderQuestion()}else completeRegion()},850)}
 function completeRegion(){state.done.add(state.current.id);state.completed++;updateStatus();if(state.completed===4){clearInterval(state.interval);winGame();return}stage.innerHTML=`<div class="game-final"><div class="emoji">🎯</div><h3>${state.current.name} completado</h3><p>¡Bien! Ya llevas <b>${state.completed}/4 regiones</b>. Regresa al mapa para continuar.</p><button class="restart-btn" id="continueMap">Continuar expedición</button></div>`;stage.querySelector('#continueMap').onclick=setMap}
 function winGame(){state.locked=true;stage.innerHTML=`<div class="game-final" id="winPanel"><div class="emoji">🏆</div><h3>¡EXPEDICIÓN COMPLETADA!</h3><p>Superaste Valle del Cauca, Eje Cafetero, Páramo y Bogotá con tus vidas restantes.</p><p>🎉 ¡Los Andes son tuyos!</p><button class="restart-btn" id="restartGame">Jugar otra vez</button></div>`;stage.querySelector('#restartGame').onclick=restart;confetti(stage.querySelector('#winPanel'))}
 function confetti(parent){const pieces=70;for(let i=0;i<pieces;i++){const el=document.createElement('i');el.className='confetti-piece';el.style.left=`${Math.random()*100}%`;el.style.setProperty('--drift',`${(Math.random()-.5)*260}px`);el.style.animationDelay=`${Math.random()*1.2}s`;el.style.animationDuration=`${2+Math.random()*1.8}s`;el.style.transform=`rotate(${Math.random()*180}deg)`;el.style.background=['#dced92','#f3c74f','#7ac6d8','#ef8f8f','#ffffff'][Math.floor(Math.random()*5)];parent.appendChild(el)}}
 updateStatus();setMap();
}
